...
type WebAccessLogsApiService service

/*
WebAccessLogsApiService Search Web Access logs.
Searches your tenant Access logs using the Elasticsearch query language. Timeframe between from_date to to_date is limited to 7 days. Results are always sorted by date in ascending order.
 * @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param optional nil or *LogsAccessPostOpts - Optional Parameters:
     * @param "Body" (optional.Interface of LogSearch) -

@return AccessLogSearchResults
*/

type LogsAccessPostOpts struct {
	Body optional.Interface
}

func (a *WebAccessLogsApiService) LogsAccessPost(ctx context.Context, localVarOptionals *LogsAccessPostOpts) (AccessLogSearchResults, *http.Response, error) {
	var (
		localVarHttpMethod  = strings.ToUpper("Post")
		localVarPostBody    interface{}
		localVarFileName    string
		localVarFileBytes   []byte
		localVarReturnValue AccessLogSearchResults
	)

	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/logs/access"
	...
	if localVarOptionals != nil && localVarOptionals.Body.IsSet() {
		...
		localVarOptionalBody, localVarOptionalBodyok := localVarOptionals.Body.Value().(LogSearch)
		if !localVarOptionalBodyok {
			return localVarReturnValue, nil, reportError("body should be LogSearch")
		}
