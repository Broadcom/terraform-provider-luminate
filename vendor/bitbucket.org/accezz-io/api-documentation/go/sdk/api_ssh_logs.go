...
type SSHLogsApiService service

/*
SSHLogsApiService Search SSH logs
Searches your tenant SSH logs using the Elasticsearch query language. Timeframe between from_date to to_date is limited to 7 days. Results are always sorted by date in ascending order.
 * @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param optional nil or *LogsSshPostOpts - Optional Parameters:
     * @param "Body" (optional.Interface of LogSearch) -

@return SshLogSearchResults
*/

type LogsSshPostOpts struct {
	Body optional.Interface
}

func (a *SSHLogsApiService) LogsSshPost(ctx context.Context, localVarOptionals *LogsSshPostOpts) (SshLogSearchResults, *http.Response, error) {
	var (
		localVarHttpMethod  = strings.ToUpper("Post")
		localVarPostBody    interface{}
		localVarFileName    string
		localVarFileBytes   []byte
		localVarReturnValue SshLogSearchResults
	)

	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/logs/ssh"
