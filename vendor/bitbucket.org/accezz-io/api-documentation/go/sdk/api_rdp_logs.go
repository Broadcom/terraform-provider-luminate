...
type RDPLogsApiService service

/*
RDPLogsApiService Search RDP logs.
Searches your tenant RDP logs using the Elasticsearch query language. Timeframe between from_date to to_date is limited to 7 days. Results are always sorted by date in ascending order.
 * @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param optional nil or *LogsRdpPostOpts - Optional Parameters:
     * @param "Body" (optional.Interface of LogSearch) -

@return RdpLogSearchResults
*/

type LogsRdpPostOpts struct {
	Body optional.Interface
}

func (a *RDPLogsApiService) LogsRdpPost(ctx context.Context, localVarOptionals *LogsRdpPostOpts) (RdpLogSearchResults, *http.Response, error) {
	var (
		localVarHttpMethod  = strings.ToUpper("Post")
		localVarPostBody    interface{}
		localVarFileName    string
		localVarFileBytes   []byte
		localVarReturnValue RdpLogSearchResults
	)

	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/logs/rdp"
